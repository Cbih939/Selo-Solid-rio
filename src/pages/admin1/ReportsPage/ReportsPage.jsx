...
const OngReportsPage = () => {
  ...
  const handleOpenUserProfile = async (user) => {
    ...
    try {
      // Busca o perfil detalhado para ter as infos do mapeamento social
      ...
      // Parse de campos JSON do banco, caso venham como string
      try {
  ...
  // ++ FUNÇÃO ATUALIZADA: GERAR RELATÓRIO PDF PROFISSIONAL (SEM LOGO_BASE64) ++
  const handleDownloadUserDossier = () => {
    ...
    // Determina o nome da OSC para o cabeçalho
    ...
    // 1. Cabeçalho com Texto Simples e Profissional
    doc.setFontSize(12);
    doc.setTextColor(234, 88, 12); // Laranja Selo Cidadania
    ...
    // Montando Tabela 2: Mapeamento Social
    autoTable(doc, {
    ...
    // Dependentes Completos
    const deps = p.dependents || [];
    const dependentsBody = deps.map(d => [
      d.full_name || d.name,
      formatDateOnly(d.birth_date),
      d.cpf || '-',
      d.kinship || '-'
    ]);
    ...
    autoTable(doc, {
      startY: doc.lastAutoTable.finalY + 8,
      head: [['Nome Dependente', 'Data Nascimento', 'CPF Dependente', 'Parentesco']],
    ...
    // Provas Sociais com Avaliador e Feedback
    const proofsBody = userProofs.map(proof => [
      proof.title,
      formatDateOnly(proof.created_at),
    ...
    autoTable(doc, {
      startY: doc.lastAutoTable.finalY + 8,
      head: [['Atividade/Prova Social', 'Data Envio', 'Status', 'Avaliado por', 'Feedback do Avaliador']],
      body: proofsBody.length > 0 ? proofsBody : [['Nenhuma prova social enviada.', '', '', '', '']],
      theme: 'grid',
      headStyles: { fillColor: [71, 85, 105] }, // Cinza Ardósia
      styles: { fontSize: 8 }
    });
    ...
    autoTable(doc, {
      startY: doc.lastAutoTable.finalY + 8,
      head: [['Custo/Resgate Realizado', 'Data do Resgate', 'Custo em Selos']],
      body: redemptionsBody.length > 0 ? redemptionsBody : [['Nenhum resgate efetuado por este utilizador.', '', '']],
      theme: 'grid',
  ...
  if (loading && !reportData) return <ContentWrapper title="Relatórios"><p>A carregar relatórios...</p></ContentWrapper>;
    ...
    <ContentWrapper title="Relatórios e Auditoria">
      ...
      <div className={styles.headerBlock}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            <h2 className={styles.mainTitle}>Análise e Dossiês</h2>
            <p className={styles.introText}>
              Acompanhe as métricas de impacto da sua OSC. Gere relatórios em PDF, audite a distribuição de selos e acesse o dossiê completo de cada beneficiário.
            </p>
          </div>
        </div>
      </div>
      ...
      {data ? (
        <>
          ...
          <div className={styles.reportBlock}>
            <ReportSection title="Diretório de Beneficiários (A-Z)">
      ...
      {/* ++ NOVO MODAL CUSTOMIZADO: VISÃO 360 ++ */}
