// Arquivo: pages/user/MySocialProofsPage/MySocialProofsPage.jsx (CORRIGIDO)
...
const MySocialProofsPage = ({ user }) => {
  ...
  const fetchUserProofs = useCallback(async () => {
    if (user) {
      setLoading(true);
      try {
        // Certifique-se que o backend retorna o campo 'activity_name' ou 'title' no JSON
        ...
        {proofs.length > 0 ? proofs.map(proof => (
          <li key={proof.id} className={styles.listItem}>
            <div className={styles.proofInfo}>
              {/* CORREÇÃO AQUI: Usando description da atividade ou fallback para não aparecer texto fantasma */}
              <p className={styles.title}>
                {proof.activity_description || proof.title || "Atividade Social"}
              </p>
            ...
            <div className={styles.actions}>
              {proof.feedback_message && (
                <button className={styles.iconButton} title="Ver Feedback" onClick={() => { setSelectedProof(proof); setMessageModalOpen(true); }}>
                  <Icon path={ICONS.message} />
                </button>
              )}
              {proof.status === 'pending' && (
                <button className={styles.iconButton} title="Editar" onClick={() => handleEditClick(proof)}>
