// Arquivo: pages/ReportsPage/ReportsPage.jsx (Versão Final Corrigida e Traduzida)
...
// ### ATUALIZAÇÃO: Dicionário de traduções centralizado ###
...
const ReportsPage = ({ user }) => {
  ...
  // ### LÓGICA CORRIGIDA: Define o estado inicial baseado no perfil do usuário ###
  const [selectedOng, setSelectedOng] = useState(user && user.role === 'ong' ? user.ong_id : 'all');
  ...
  useEffect(() => {
    // Busca ONGs apenas se o usuário for admin
    if (user && user.role === 'admin5') {
  ...
  }, [user]); // Dependência corrigida para 'user'
  ...
  }, [ongSearchTerm, ongs, user]); // Dependência corrigida para 'user'

  useEffect(() => {
    const fetchReportData = async () => {
      setLoading(true);
      try {
        // ### ATUALIZAÇÃO: Parâmetro de busca corrigido para 'search' ###
        const params = {
  ...
  const generatePDF = () => {
    ...
    // ### ATUALIZAÇÃO: Cabeçalhos e dados do PDF traduzidos e formatados ###
  ...
  // ### ATUALIZAÇÃO: Simplificado o acesso aos dados, removendo aninhamentos ###
  const data = reportData;

  return (
    <ContentWrapper title="Relatórios">
      {/* Filtros de OSC visíveis apenas para admin */}
      {user && user.role === 'admin5' && (
            ...
            <ReportSection title="Estatísticas Gerais">
              <div className={styles.mainGrid}>
                <div className={styles.statsColumn}>
                  <div className={styles.statCard} style={{ backgroundColor: '#dbeafe' }}><p>Total de Beneficiários</p><span>{data.generalStats?.totalUsers || 0}</span></div>
                  <div className={styles.statCard} style={{ backgroundColor: '#e0f2fe' }}><p>Selos em Circulação</p><span>{data.generalStats?.totalSealsInCirculation || 0}</span></div>
          ...
          <div className={styles.reportBlock}>
            <ReportSection title="Beneficiários Cadastrados">
              <div className={styles.sectionHeader}>
                <InputField label="Pesquisar Beneficiário" placeholder="Nome ou CPF do beneficiário..." value={userSearchTerm} onChange={(e) => setUserSearchTerm(e.target.value)} />
              </div>
